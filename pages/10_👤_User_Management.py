...
def update_user(user_id: int, email: str, role: str, employee_id: int = None) -> tuple:
    """Update user details"""
    try:
        # Check email
...
if st.session_state.get('show_create_form', False):
    ...
    with st.form("create_user_form", clear_on_submit=True):
        ...
        with col2:
            new_email = st.text_input("Email *", max_chars=255)
            new_role = st.selectbox("Role *", AVAILABLE_ROLES, index=AVAILABLE_ROLES.index('viewer'))
            
            # Employee dropdown
            emp_options = ['-- No Employee Link --'] + employees_df['full_name'].tolist()
            new_employee = st.selectbox("Link to Employee", emp_options)
        ...
        if submitted:
            ...
            if errors:
                ...
                emp_id = None
                if new_employee != '-- No Employee Link --':
                    emp_row = employees_df[employees_df['full_name'] == new_employee]
